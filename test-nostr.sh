#!/bin/bash
# Quick nostr round-trip test using the RipperClaw Rust backend
# Generates keys, publishes a test loadout, fetches it back

cd ~/ripperdoc/app/src-tauri

cat > src/test_nostr.rs << 'RUST'
...
# Add binary target to Cargo.toml if not present
if ! grep -q "test_nostr" Cargo.toml; then
    cat >> Cargo.toml << 'TOML'

[[bin]]
name = "test_nostr"
path = "src/test_nostr.rs"
TOML
fi

echo "Building test binary..."
cargo build --bin test_nostr --release 2>&1 | tail -5

echo ""
echo "Running test..."
./target/release/test_nostr
