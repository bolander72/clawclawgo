...
/**
 * ripperdoc — Cyberware manager for AI agents
 *
 * Commands:
...
async function cmdDiff() {
  ...
  if (!theirFile) {
    console.error("Usage: ripperdoc diff <loadout.json>");
    process.exit(1);
  }
...
async function cmdInspect() {
  const file = args[1];
  if (!file) {
    console.error("Usage: ripperdoc inspect <loadout.json>");
    process.exit(1);
  }
...
async function cmdHelp() {
  console.log(`
  ${"\x1b[1m"}ripperdoc${"\x1b[0m"} — Cyberware manager for AI agents
